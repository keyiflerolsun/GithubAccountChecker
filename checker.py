...
class GitChecker(object):
    ...
    def giris_yap(self, posta, sifre, cikti) -> None:
        ...
        payload_icin = BeautifulSoup(oturum.get(self.referer, headers=self.headers).content, "lxml")
        ...
        try:
            oturum.post('https://github.com/session', headers=self.headers, data=payload).cookies['user_session']
        except KeyError:
            return None
